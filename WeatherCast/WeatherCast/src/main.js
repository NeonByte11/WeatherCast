...
constructor() {
    ...
    this.apiKey = '49cc8c821cd2aff9af04c9f98c36eb74';

    // Set the base URLs for fetching weather data
    ...
    // Set the units for temperature data (imperial in this case)
    this.units = 'imperial';

    // Select the geolocation button from the HTML document
	...
	displayCurrentTemperature: function (response) {
		if (response.status == 200) {
			...
			const cityBackgrounds = {
				'New York': 'url(https://unsplash.com/photos/aerial-view-of-city-buildings-during-daytime-dHU_8dceFc8)',
				'Los Angeles': 'url(/assets/la-bg.jpg)',
				'Dhaka': 'url(https://unsplash.com/s/photos/dhaka-city?orientation=landscape)',
				// Add more city backgrounds here
			  };
			  ...
			  function changeBackgroundImage(cityName) {
				...
				if (backgroundImage) {
				  ...
				  const unsplashUrl = `http://source.unsplash.com/1600x900/?${cityName}`;
				//    or u can use following way
				//   const unsplashUrl = document.body.style.backgroundImage = "url('http://source.unsplash.com/1600x900/?" + cityName + " ')"
				  fetch(unsplashUrl)
					.then(response => {
					  if (response.ok) {
						document.body.style.backgroundImage = `url(${unsplashUrl})`;
					  } else {
						console.log('No background image found for this city.');
					  }
					})
					.catch(error => {
...
const OPENWEATHER_KEY = '49cc8c821cd2aff9af04c9f98c36eb74';
